...
load_det_data <- function() {
  VIEWNAME <- "V_W_TL_IN_CRF_DET_LOAD"
  if (CSV.INPUT) {
    det <- read_raw_data_from_csv(VIEWNAME)
  }
  else {
    det <- read_db_table(VIEWNAME)
  }
  cat("Loaded", nrow(det), "rows from", VIEWNAME, "\n", file=log_con)
  
  #Remove -ve values
  det <- det[det$RESULT >=0,]
...
load_study_data <- function() {
  ...
  # Taking care of studies with DeVELOPMENT_PHASE such as IIa, IIb, etc
  x <- as.character(Studies$DEVELOPMENT_PHASE)
  x <- toupper(x)
  x <- gsub("IA","I",x); x <- gsub("IB","I",x); x <- gsub("IC","I",x); x <- gsub("ID","I",x);
  #Restricting it to only Phase-I, II, II, IV
  Studies$DEVELOPMENT_PHASE <- x
  Studies <- Studies[Studies$DEVELOPMENT_PHASE %in% c("PHASE I", "PHASE II", "PHASE III", "PHASE IV"),]
  Studies$DEVELOPMENT_PHASE <- factor(Studies$DEVELOPMENT_PHASE)
...
#Keeping track of rownames
 ...
 x <- strsplit(as.vector(as.character(Staff$STATUS_DATE))," "); Staff$STATUS_DATE <- as.Date(sapply(x,"[",1));
...
#  write.table(Staff,file="nosamedayturnover",sep='\t',quote=F,row.names=F)
################################################################################################################
