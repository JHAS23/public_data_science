...
create_staff_turnover_dataset_one_site <- function(site.id, site.date, module) {
  ...
  Staff <- Staff[order(Staff$HM_PERSON_ID,
                       ...
                       -Staff$HM_SITE_PERSONNEL_TURNOVER_ID),]
  ...
  # Exclude any statuses from before the run-as-of date
...
create_staff_vars <- function(Data, Staff.turnover, module) {
  ...
  if (module == "TO") {
    # Merge PERIOD_START_1YR into data
    Staff.turnover <- merge(Staff.turnover, Data[c("HM_SITE_ID", "PERIOD_START_1YR")])
    ...
    Staff.turnover$STAFF_COUNT_MONITORS_1YR <- (
        !is.na(Staff.turnover$FIRST_ACTIVE_DATE) & 
        Staff.turnover$FIRST_ACTIVE_DATE < Staff.turnover$DATE &
          Staff.turnover$ROLE == "MONITOR" &
              ((!is.na(Staff.turnover$LAST_ACTIVE_DATE) &
                  Staff.turnover$LAST_ACTIVE_DATE > Staff.turnover$PERIOD_START_1YR)
               | Staff.turnover$CURRENTLY_ACTIVE
               | is.na(Staff.turnover$LAST_INACTIVE_DATE)
               | Staff.turnover$LAST_INACTIVE_DATE > Staff.turnover$PERIOD_START_1YR)
    )
    ...
    Staff.turnover$NUM_MONITORS_LEFT_1YR <- (
      Staff.turnover$STAFF_COUNT_MONITORS_1YR &
      ! Staff.turnover$CURRENTLY_ACTIVE &
        Staff.turnover$LAST_INACTIVE_DATE > pmax(Staff.turnover$PERIOD_START_1YR, 
                                                 Staff.turnover$FIRST_ACTIVE_DATE)
      )
...
create_tl_variables <- function(Data){
  #Create variables for the new TL module
  #List of variables = #visit, #pages, #active subjects, filter sites, TA, Region
  # Variables from PD and SA
  
  d <- Data[Data$SITE_STATUS %in% "ONGOING",]
  d <- d[,c("HM_SITE_ID","HM_STUDY_ID","DATE","DEVELOPMENT_PHASE","STUDY_THERAPEUTIC_AREA","PFIZER_PROTOCOL_ID",
            "DAYS_SINCE_STUDY_FSFV","NUM_STUDY_SITES","UNMONITORED_SITE_DAYS","REGION")]
  monthid <- as.integer(format(d$DATE[1], "%Y%m"))
  
  #For Running purposes
  #monthid <- monthid -1
  
  #Merge with the input det data
  q <- INPUT$det
#  write.table(q,file="beforeCheck",sep='\t',quote=F,row.names=F)
  #Convert to Integer
  q$MONTH_ID <- as.integer(as.character(q$MONTH_ID))
  #write.table(q,file="afterCheck",sep='\t',quote=F,row.names=F)
  #Filter Data
  q <- q[q$MONTH_ID == monthid,]
  #write.table(monthid,file="monthid",sep='\t',quote=F,row.names=F,col.names=F)
  #write.table(q,file="afterCheck",sep='\t',quote=F,row.names=F)
  #Add Pfizer Protocol Id
  q <- merge(q,d[,c("HM_SITE_ID","PFIZER_PROTOCOL_ID")])
  if(nrow(q)==0) {
    cat("No DET data  present for ", monthid, "\n", file=log_con)
  }
  
  # Number of active subjects
  t0 <- aggregate(q$SUBJECT_ID,by=list(q$HM_SITE_ID),
                  FUN=function(SUBJECT_ID){ 
                    return(length(unique(SUBJECT_ID)))
                  })
  names(t0) <- c("HM_SITE_ID","num_active_pat")
  
  # Number of pages
  t1 <- aggregate(q$DATA_OBJECT_COUNT,by=list(q$HM_SITE_ID),
                 FUN=function(DATA_OBJECT_COUNT){ 
                   return(sum(DATA_OBJECT_COUNT))
                 })
  names(t1) <- c("HM_SITE_ID","num_pages")
  
  # Number of visits
  t2 <- aggregate(q$HM_SITE_ID,by=list(q$HM_SITE_ID),
                  FUN=function(HM_SITE_ID){ 
                    return(length(HM_SITE_ID))
                  })
  names(t2) <- c("HM_SITE_ID","num_visits")
  
  # Median Days
  calc_median_days <- function(x,y){
    m <- c()
    for(i in 1:length(x)){
      k <- rep(y[i],x[i])
      m <- append(m,k)
  ...
  w <- unique(q$HM_SITE_ID)
  z <- c()
  for(i in 1:length(w)){
    x <- q[q$HM_SITE_ID==w[i],"DATA_OBJECT_COUNT"]
    y <- q[q$HM_SITE_ID==w[i],"RESULT"]
    z[i] <- calc_median_days(x,y)
  }
  t3 <- data.frame(HM_SITE_ID=w,det_median_days=z)
  
  #Aggregating different data frames
  t <- merge(t0,t1)
  t <- merge(t,t2)
  t <- merge(t,t3)
  
  
  #Missing DET Data Stats
  x <-  length(unique(d$HM_SITE_ID)) - length(intersect(d$HM_SITE_ID,q$HM_SITE_ID))
  missing_studies <- setdiff(d$PFIZER_PROTOCOL_ID,q$PFIZER_PROTOCOL_ID)
  missing_studies <- paste(missing_studies,collapse=", ")
  cat("TL Module DET Data not available for ", paste(x,"ongoing sites"), "\n",file=log_con)
  cat("TL Module DET Data not available for following Studies:\n ", paste0(missing_studies), "\n",file=log_con)
  d <- merge(d,t)
  cat("TL Module DET Data available for ", paste(nrow(d),"ongoing sites"), "\n",file=log_con)
  
  
  #Therapeutic Area
  x <- as.character(d$STUDY_THERAPEUTIC_AREA)
  x <- ifelse(x %in% c("NEUROSCIENCE (NEUROLOGY)","NEUROSCIENCE (PSYCHIATRY)"), "NEUROSCIENCE",x)
  x <- ifelse(x %in% c("CARDIOVASCULAR","INFLAMMATION","ONCOLOGY","PAIN","VACCINES","IMMUNOLOGY","METABOLIC DISEASE"), x, "OTHERS")
  d$STUDY_THERAPEUTIC_AREA <- factor(x)
  
  return(d)
  
}

sa_pd_vars <- function(tl,pd,sa){
  
  # From SA
  sa <- sa[,c("HM_SITE_ID", "MONITOR_TO_PATIENT_RATIO", "NUM_AES_REPORTED_PREV_12MON", "PCT_ACTIVE_SUBJECTS", "PCT_DISCONTINUED_SUBJECTS")]
  tl <- merge(tl,sa)
  
  # From PD
  pd <- pd[,c("HM_SITE_ID", "DATE", "NUM_PDS_IN_VISIT")]
  ## Get the latest PD count for each site
  pd <- pd[order(pd$HM_SITE_ID, pd$DATE),]
  pd <- pd[c(pd[1:(nrow(pd)-1),"HM_SITE_ID"] != pd[2:nrow(pd),"HM_SITE_ID"], TRUE),]
  pd <- pd[,c("HM_SITE_ID", "NUM_PDS_IN_VISIT")]
  names(pd)[2] <- "PD_COUNT_ACTUAL_CURRENT"
  
  tl <- merge(tl,pd,all.x=T)
  
  # Remove any potential discrepancies
  tl$DEVELOPMENT_PHASE <- factor(as.character(tl$DEVELOPMENT_PHASE))
  tl$STUDY_THERAPEUTIC_AREA <- factor(as.character(tl$STUDY_THERAPEUTIC_AREA))
  tl$REGION <- factor(as.character(tl$REGION))
  tl$PFIZER_PROTOCOL_ID <- factor(as.character(tl$PFIZER_PROTOCOL_ID))
  tl <- tl[!is.na(tl$DEVELOPMENT_PHASE) & !is.na(tl$STUDY_THERAPEUTIC_AREA) & !is.na(tl$REGION),]
  #tl[is.na(tl)] <- 0
  tl$MONITOR_TO_PATIENT_RATIO <- ifelse(is.na(tl$MONITOR_TO_PATIENT_RATIO),0,tl$MONITOR_TO_PATIENT_RATIO)
  tl$NUM_AES_REPORTED_PREV_12MON <- ifelse(is.na(tl$NUM_AES_REPORTED_PREV_12MON),0,tl$NUM_AES_REPORTED_PREV_12MON)
  tl$PCT_ACTIVE_SUBJECTS <- ifelse(is.na(tl$PCT_ACTIVE_SUBJECTS),0,tl$PCT_ACTIVE_SUBJECTS)
  tl$PCT_DISCONTINUED_SUBJECTS <- ifelse(is.na(tl$PCT_DISCONTINUED_SUBJECTS),0,tl$PCT_DISCONTINUED_SUBJECTS)
  tl$PD_COUNT_ACTUAL_CURRENT <- ifelse(is.na(tl$PD_COUNT_ACTUAL_CURRENT),0,tl$PD_COUNT_ACTUAL_CURRENT)
  names(tl) <- toupper(names(tl))
  
  # Transformation
  ## Output 2 dataframes - 1 transformd and 1 original
  Model.data.TL.org <- tl
  ## Continuous Variables that need to be transformed
  cont0 <- c("MONITOR_TO_PATIENT_RATIO", "NUM_AES_REPORTED_PREV_12MON", "PCT_ACTIVE_SUBJECTS", "DAYS_SINCE_STUDY_FSFV", "UNMONITORED_SITE_DAYS", "NUM_STUDY_SITES", 
            "PCT_DISCONTINUED_SUBJECTS", "PD_COUNT_ACTUAL_CURRENT", "NUM_PAGES", "NUM_VISITS")
  ## Variables that should not be transformed
  oth <- c("DET_MEDIAN_DAYS","DEVELOPMENT_PHASE","STUDY_THERAPEUTIC_AREA","REGION")
  
  
  # Selection of Variables to be transformed. Any variable with a max value of >1 will be transformed
  ## Transformation: scale and center, so that the mean is 0 and sd=1
  m0 <- tl
  x <- apply(m0[,cont0],2,min)
  y <- apply(m0[,cont0],2,max)
  z <- apply(m0[,cont0],2,var)
  q <- data.frame(predictor=names(x),minimum=as.vector(x),maximum=y, variance=z)
  x <- as.character(q[q$maximum >1,"predictor"])
  t <- data.frame(scale(m0[,x],center = TRUE, scale = TRUE))
  Model.data.TL.tfm <- cbind(t,tl[,setdiff(names(tl),names(t))])

  return(list(Model.data.TL.org=Model.data.TL.org, Model.data.TL.tfm=Model.data.TL.tfm))
}
